...
if($busca == ""){
	...
	$query = $pdo->query("SELECT * from $tabela where (nome LIKE '%$busca%' OR codigo LIKE '%$busca%') and ativo = 'Sim' order by id asc");
}
...
<div class="widget" style="">	
						<a href="#" onclick="produto('{$id}', '{$nome}', '$valor_venda', '$codigo')">		
							<div class="r3_counter_box" style="min-height: 60px; padding:10px">
								...
								<div class="stats">
					<p style="font-size:12px">{$nomeF} {$nome_subF}	</p>
					<span><span style="color:red; font-size:11px">R$ {$valor_vendaF}</span> <span class="" style="color:#000; font-size:11px">({$estoque}) Itens</span></span>
					</div>	
							</div>
							</a>
						</div>
