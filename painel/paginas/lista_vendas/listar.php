...
if($alterou_data == 'Sim'){
	if($dataInicial != "" || $dataFinal != ""){
		$query = $pdo->query("SELECT * from $pagina where (data_venc >= '$dataInicial' and data_venc <= '$dataFinal') and pago LIKE '$status' and referencia = 'Venda' order by id desc ");
	}
}else if($status != '%%' and $alterou_data == ''){
	$query = $pdo->query("SELECT * from $pagina where pago LIKE '$status' and referencia = 'Venda' order by id desc ");
}

else if($vencidas == 'Vencidas'){
	$query = $pdo->query("SELECT * from $pagina where data_venc < curDate() and pago = 'Não' and referencia = 'Venda' order by id desc ");
}
	...
	$query = $pdo->query("SELECT * from $pagina where data_venc = '$data_amanha' and pago = 'Não' and referencia = 'Venda' order by id desc ");
}
else if($vencidas == 'Canceladas'){
	$query = $pdo->query("SELECT * from $pagina where cancelada = 'Sim' and referencia = 'Venda' order by id desc ");
}
else{
	$query = $pdo->query("SELECT * from $pagina WHERE data_venc = curDate() and referencia = 'Venda' order by id desc ");
}
...
echo <<<HTML
			...
			<div class="dropdown head-dpdn2" style="display: inline-block;">                      
                        <a class="icones_mobile" title="Cancelar Venda" href="#" aria-expanded="false" aria-haspopup="true" data-bs-toggle="dropdown" class="dropdown {$ocultar_cancelar}"><i class="fa fa-ban text-danger"></i> </a>
					...
					<big><a class="icones_mobile" class="{$ocultar}" href="#" onclick="parcelar('{$id}', '{$valor}', '{$descricao}')" title="Parcelar Conta"><i class="fa fa-calendar-o " style="color:#7f7f7f"></i></a></big>

					<big><a class="icones_mobile" class="{$ocultar}" href="#" onclick="baixar('{$id}', '{$valor}', '{$descricao}', '{$saida}')" title="Baixar Conta"><i class="fa fa-check-square " style="color:#079934"></i></a></big>
					...
					<big><a class="icones_mobile" href="rel/comprovante.php?id={$id}&imprimir=Não" target="_blank" title="Ver Comprovante"><i class="fa fa-file-pdf-o " style="color:green"></i></a></big>
				</td>  
			</tr> 
HTML;
}
